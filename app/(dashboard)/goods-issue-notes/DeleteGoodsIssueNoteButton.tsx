...
type DeleteGoodsIssueNoteButtonProps = {
  ginId: number;
  ginNumber: string;
...
const DeleteGoodsIssueNoteButton = ({
  ginId,
  ginNumber,
  redirectTo = "/goods-issue-notes",
  compact = false,
}: DeleteGoodsIssueNoteButtonProps) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch(`/api/goods-issue-notes/${ginId}`, {
        method: "DELETE",
      });
      const result = (await response.json()) as { error?: string };
      if (!response.ok) {
        throw new Error(result.error ?? "Failed to delete goods issue note.");
      }
    },
    onSuccess: () => {
      setIsOpen(false);
    ...
    onError: (mutationError) => {
      setError(
        mutationError instanceof Error
          ...
          : "Failed to delete goods issue note.",
      );
    },
  });
  ...
  return (
    <>
      ...
      >
        ...
        {compact ? "Delete" : "Delete GIN"}
      </button>
      {isOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-xl border border-stone-200 bg-white p-5">
            <h3 className="text-[16px] font-semibold text-stone-900">Delete {ginNumber}?</h3>
            <p className="mt-2 text-[13px] leading-relaxed text-stone-600">
              This reverses issued stock and invoice issued quantities, then soft-deletes the goods issue note.
            </p>
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              >
                {deleteMutation.isPending ? "Deleting..." : "Delete GIN"}
              </button>
            </div>
          </div>
        </div>
...
export default DeleteGoodsIssueNoteButton;
