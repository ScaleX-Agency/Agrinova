...
const DeleteReceiptButton = ({
  ...
  const deleteMutation = useMutation({
    ...
    onError: (mutationError) => {
      setError(mutationError instanceof Error ? mutationError.message : "Failed to delete receipt.");
    },
  });
  ...
  return (
    <>
      ...
      >
        ...
        {compact ? "Delete" : "Delete Receipt"}
      </button>
      {isOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-xl border border-stone-200 bg-white p-5">
            <h3 className="text-[16px] font-semibold text-stone-900">Delete {receiptNo}?</h3>
            <p className="mt-2 text-[13px] leading-relaxed text-stone-600">
              This deletes the receipt and also soft-deletes linked settlements and related commissions together.
            </p>
